...
async function withRetry(fn, opts = {}) {
  const {
    maxAttempts = 3, baseDelayMs = 500, maxDelayMs = 15000,
    jitter = 0.2, shouldRetry = defaultShouldRetry, label = "retry",
  } = opts;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try { return await fn(attempt); }
      ...
      if (!doRetry) { console.error(`[${label}] Falhou definitivamente na tentativa ${attempt}:`, err.message); throw err; }
