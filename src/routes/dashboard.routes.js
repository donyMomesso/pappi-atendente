...
router.get("/orders/kanban", authDash, async (req, res) => {
  try {
    ...
    const columns = {
      ...
      dispatched: [], concluded: [], cancelled: [], cw_failed: [],
    };
    ...
    for (const o of orders) {
      ...
      columns[col].push({
        id: o.id, cwOrderId: o.cwOrderId,
        customerName: o.customer?.name || o.customer?.phone || "—",
        total: o.total, fulfillment: o.fulfillment,
        paymentMethodName: o.paymentMethodName, createdAt: o.createdAt, items,
      });
    }
    res.json(columns);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
router.get("/settings", authAdmin, async (req, res) => {
  try {
    ...
    res.json({
      id: tenant.id, name: tenant.name, city: tenant.city,
      waPhoneNumberId: tenant.waPhoneNumberId, cwBaseUrl: tenant.cwBaseUrl,
      cwStoreId: tenant.cwStoreId, active: tenant.active,
...
router.get("/stats/report", authAdmin, async (req, res) => {
  try {
    ...
    const [msgSent, msgReceived, orders, uniqueCustomers, handoffs, handoffsOpen, topCustomers, cwFailed] = await Promise.all([
...
router.get("/google-contacts/status",     authAdmin, async (_req, res) => res.json({ authorized: await googleContacts.isAuthorized() }));
router.post("/google-contacts/disconnect",authAdmin, async (_req, res) => { await googleContacts.disconnect(); res.json({ ok: true }); });
router.get("/google-contacts/search",     authDash,  async (req, res) => { const q = (req.query.q || "").trim(); if (!q) return res.json([]); res.json(await googleContacts.searchContacts(q)); });
...
router.get("/google-contacts/callback", async (req, res) => {
  try {
    ...
    res.send(`<html><body style="font-family:sans-serif;text-align:center;padding:40px"><h2>✅ Google Contacts autorizado!</h2><script>setTimeout(()=>window.close(),3000)</script></body></html>`);
  } catch (err) { res.status(500).send("Erro: " + err.message); }
});

router.get("/retention/campaigns",         authAdmin, async (req, res) => { try { const tenantId = req.query.tenant || "tenant-pappi-001"; res.json(await prisma.retentionCampaign.findMany({ where: { tenantId }, orderBy: { createdAt: "desc" } })); } catch (err) { res.status(500).json({ error: err.message }); } });
router.post("/retention/campaigns",        authAdmin, async (req, res) => { try { const tenantId = req.query.tenant || req.body.tenantId || "tenant-pappi-001"; const { name, message, delayHours = 20, monthlyLimit = 100, aiFilter = true } = req.body; if (!name || !message) return res.status(400).json({ error: "name e message obrigatórios" }); res.json(await prisma.retentionCampaign.create({ data: { tenantId, name, message, delayHours: +delayHours, monthlyLimit: +monthlyLimit, aiFilter: !!aiFilter } })); } catch (err) { res.status(500).json({ error: err.message }); } });
router.patch("/retention/campaigns/:id",   authAdmin, async (req, res) => { try { const data = {}; for (const k of ["name","message","delayHours","monthlyLimit","aiFilter","active"]) if (req.body[k] !== undefined) data[k] = typeof req.body[k] === "boolean" ? req.body[k] : (k === "active" || k === "aiFilter" ? !!req.body[k] : (["delayHours","monthlyLimit"].includes(k) ? +req.body[k] : req.body[k])); res.json(await prisma.retentionCampaign.update({ where: { id: req.params.id }, data })); } catch (err) { res.status(500).json({ error: err.message }); } });
router.delete("/retention/campaigns/:id",  authAdmin, async (req, res) => { try { await prisma.retentionCampaign.delete({ where: { id: req.params.id } }); res.json({ ok: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
router.post("/retention/run",              authAdmin, async (_req, res) => { retention.runAll().catch(err => console.error("[Retention] run manual:", err.message)); res.json({ ok: true }); });
router.get("/retention/stats",             authAdmin, async (req, res) => { try { res.json(await retention.getMonthlyStats(req.query.tenant || "tenant-pappi-001")); } catch (err) { res.status(500).json({ error: err.message }); } });

router.get("/wa-internal/status",    authDash,  (_req, res) => res.json(baileys.getAllStatuses()));
router.post("/wa-internal/connect",  authAdmin, async (req, res) => { try { await baileys.start(req.body.instanceId || "default"); res.json({ ok: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
router.post("/wa-internal/disconnect",authAdmin,(req, res) => { baileys.disconnect(req.body.instanceId || "default"); res.json({ ok: true }); });
router.patch("/wa-internal/numbers", authAdmin,(req, res) => { baileys.setNotifyNumbers(Array.isArray(req.body.numbers) ? req.body.numbers : [], req.body.instanceId || "default"); res.json({ ok: true }); });
...
router.get("/debug", authAdmin, async (req, res) => {
  try {
    ...
    const [tenant, recentCustomers] = await Promise.all([
      ...
      prisma.customer.findMany({ where: { tenantId }, orderBy: { lastInteraction: "desc" }, take: 5, select: { phone: true, name: true, lastInteraction: true } }),
    ]);
    res.json({
...
router.get("/whatsapp/templates", authDash, async (req, res) => {
  try { const { wa } = await getClients(req.query.tenant || "tenant-pappi-001"); res.json(await wa.getTemplates()); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post("/whatsapp/send-template", authDash, async (req, res) => {
  try {
    ...
    const { wa } = await getClients(tenantId);
    res.json({ ok: true, result: await wa.sendTemplate(phone, templateName, languageCode || "pt_BR", components || []) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
