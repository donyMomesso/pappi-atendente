...
async function quote(address, cw, store = {}) {
  ...
  let fee = null;
  if (cw) {
    try { fee = await cw.getDeliveryFee({ lat: geo.lat, lng: geo.lng }); } catch {}
  }

  return { lat: geo.lat, lng: geo.lng, formatted_address: geo.formatted_address, km, eta_minutes: eta, delivery_fee: fee, is_serviceable: fee !== null };
}
